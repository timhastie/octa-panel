...
- **24 Sep 2026, OCTATRICK5** (`BUILD=5 VERSION=OCTATRICK5 REMIX=tim make image-cf`): the paraphonic build -- POLY row, four voices per synth track, CHRD (DEP3 byte) and VOIC (SPD3 byte) on a cloned LFO page, chords snapped to SCALE, the engine as a DRAM unit in the 10 MB platform reserve (FREE MEM 53.5). The OS section is 1,115,838 B (3,278 B appended: loader + packed runtime) -- the FIRST of Tim's flashes to carry the loader; the same mechanism midi-scenes users flashed. Verified on a fresh emulator boot by me (see the commit). NOT flashed yet. Rule change to know: LFO 3 is muted on synth tracks in BOTH modes (a leftover CHRD value in DEP3 became a pitch LFO with POLY off; one-line revert noted in the synth README).
- **24 Sep 2026, PARAPHONIC CHORDS + the synth engine in DRAM (emulator only, not flashed; working tree left uncommitted for Tim):**
  (a) `modules/synth/poly.s` is the FM voice engine as a DRAM unit (`Linked(dram=True)`, 7,140 B linked at the arena reserve's base 0x40a955e0; `synth.s` kept for the record, no longer built). `make cf` took the DRAM unit as it was: the platform's 1,707 pages (10 MB) come off the bottom of the audio page arena (base -> 0x41495de0, 75 MB left), the loader + packed runtime are appended (3,078 B at 0x4010fdf0), `verify_dram_boot` PASSES for tim (loader 1x, fatal 0x, the reserve == the linked runtime); the unit shows FREE MEM 53.5 MB in the FLEX slot list against 63.5 on stock. ONE build change: `Detour(kind="ptr")` (build_bus.py, schema.py, MODULES.md) rewrites a 4-byte stock pointer to a symbol -- the kind table's FLEX entry 0x400d6438 -> sy_render (a Poke's write is bytes fixed before the link). The third zero run has 1,700 B free (was 168).
  (b) POLY: a sixth SEQUENCER row (OFF/ON, `#SYNTH_POLY=n`, the byte at 0x400d2cdd next to GLIDE; row-count poke 3 -> 6). With it on, a synth track has 4 voices: a voice start allocates the chord shape from the LFO page's slot 5 (CHRD: 32 shapes x 4 knob values, `po_shapes`/`po_names`, locks per step make progressions) at the PTCH word; per-voice index envelope, glide (target = V_ROOT + the PTCH delta since the voice's start, so locks/slides/LFOs move every voice) and amplitude (instant attack; sustain while the key is held or until the next trig; release tau = 5 ms * 1000^(REL/126), 127 = INF); free voice, else the oldest releasing, else the oldest sounding; each voice at 1/2, the sum saturated. The stock lifecycle stays (a live key restarts the DSP voice: use AMP ATK 0 / HOLD INF, REL to taste). Live keys: the quantizer's handler hooks (`qz_leg0` new at 0x4004fbde on the release path, `qz_leg1/2` extended) keep a held-key mask and post each key through `keys.s` (pinned at 0x400d2cb0: qz_pkey[8], qz_pmask[8]); only the last key's release posts the AMP release; legato is off while POLY is on; non-synth tracks are stock. LFO 3 muted on a poly synth track by detours at BOTH copies of the LFO engine's depth read (the routine 0x40003ca4 and the frame builder's inlined copy 0x4000d03e -- LFO 3's default PMTR is PTCH, so without it the chord byte is a pitch LFO). The LFO page: a detour at the resolver's kind-1 load 0x40031e62 returns a clone (built from the stock descriptor at first use) with slot 5 = CHRD + a name formatter, only for a synth track with POLY on; the LFO SETUP page shows the chord byte as LFO 3's DEP dial (left as is).
  MEASURED (panel 8593, a copy of the OTLIVE card, T2 = SYNTH): POLY OFF vs OCTATRICK4 on the lockstep synth8q rig: 120,090 frames, 0 samples differ. Sequencer trig with CHRD MAJ: 130.8/164.8/196.0 Hz (1 : 1.260 : 1.498); live MAJ on key 13: 261.6/329.6/392.0; MIN7 four lines; two keys held = two notes (261.6 + 174.6); the stack 13/9/6 pressed and released in turn plays [C4] [C4 G#3] [C4 G#3 F3] [G#3 F3] [F3] silence; REL 20 cuts a released key within 0.1 s (-77.7 dBFS), REL 100 lets it ring under the next key (-2.8 dB); with POLY on and DEP3 40 the lines hold 261.7-261.8 / 392.1 Hz, with POLY off the same key wanders 228-313 Hz, T7's sample swings 4,718-5,520 Hz under LFO 3; the LFO page shows CHRD only in the poly case (screens in the session scratchpad `poly/`); SYNC TO CARD + eject/insert keeps POLY and the CHRD/PTCH locks; FM SYNTH page and PLAY (-19.7 dBFS) fine. CPU per frame (engine's own, both calls, T8): POLY off 832, 1 voice 1,172, 4 voices 3,304 mean / 3,956 max, against the stock renderer's 1,273 (418 + 855) inside. Caveats: a four-note chord steals every voice (nothing rings into it); the DSP AMP envelope retriggers at each live key; a refused key trig leaves qz_pkey set once; MIDI note-offs per key untested; hardware cost and the instruction cache over the arena reserve unmeasured.
- **24 Sep 2026, follow-ups on the paraphonic build (emulator only, uncommitted):** (1) "the same key gives a different pitch with POLY off" was LFO 3: its default PMTR is PTCH and the CHRD value left in its DEP3 slot was its depth once POLY went off (191/247/318/382/298 Hz on five presses of key 13 with the slot at 36; 261.6 x5 with it at 0; the mono engine was fine). RULE CHANGED: LFO 3's depth reads 0 on a synth track in both modes (`po_lfo3_depth`; its slots are CHRD and VOIC there); verified on a fresh boot: POLY off 5 presses = 261.6 x5, after POLY on/chords/off 5 presses = 261.6 x5 and key 16 311.1 x5, GLIDE off and 64, the sequencer's single trig 130.8 each pattern; lockstep A/B vs OCTATRICK4 (POLY off): the sequencer fixture 0 of 120,090 frames differ, a pipe-driven CHROMATIC key sequence identical up to the first key and then identical at a one-frame (16-sample) lag. (2) Chords obey SCALE: `po_snap` snaps every chord note the quantizer's way (nearest degree, ties down), the mask read through the new accessor `qz_scale_mask` via the 6-byte trampoline `scale.s` pinned at 0x400d2ca8; DORIAN/PHRYGN + MAJ = 1 : 1.189 : 1.498, OFF = 1 : 1.26 : 1.498, AUG in MAJOR -> MAJ, MAJ7 in MINOR -> C D# G A#, a sequencer trig with a CHRD lock the same as a key. (3) VOIC: the LFO page's slot 2 (SPD3's byte) on a poly synth track is the voice count 1..4 (range/default in the clone, the stock enum stepper as knob handler ~4 detents a step; a Part's stock 32 reads as 4), the engine takes that many notes of the shape root-first: VOIC 2 + MAJ = root + third, 1 = root alone (mono with release tails), survives SYNC TO CARD + eject/insert; the page shows VOIC/CHRD only in the poly case. Build: out/mainos_cf.bin 1,115,838 B, 3,537 changed, unit 7,420 B, 1,572 B of cave left, verify_dram_boot PASS. Scripts/screens: scratchpad `poly/` (bug1_accept.py, voic_test2.py, chromrig.py, chrom/, voic4/, bug1d/).
