...
- **23 Sep 2026, first hardware candidate: BUILD 81 = DIRECT JUMP + SCALE QUANTIZER** (`remixes/jumpquant.py`; `BUILD=81 REMIX=jumpquant make image-cf`): `out/OCTATRACK_OCTABAM81_cf.bin` (card path, 446,744 B) / `out/OCTATRACK_OS1.40C_OCTABAM81_cf.syx` (MIDI path), copy of the OS section at `out/mainos_jumpquant_81.bin`. Verified before hand-over: the official .syx holds ONE section (id 3 MAIN OS; no bootstrap in the file), the packer round-trips it byte-identically, ours has the same single section with checksums ok and section 3 == the build; 1,657 bytes differ from stock at 36 sites, highest 0x400d7490 (< SAFE_CAVE_CEIL), none in the DSP payloads; the container version field reads ` OCTABAM80`-style (10 chars, the tool's report prints `?` for it, upstream's builds show it in SYSTEM STATUS). Emulator: boots, PLAY -25.9 dBFS, T7 [TRIG 15] -35.8, CHAIN AFTER reaches DIRECT and clamps, SCALE OFF -> PHRYGN -> OFF. NOT flashed yet (BUILD 80, quantizer alone, was built and discarded the same evening). `out/mainos_cf.bin` is back to the full tim build.
