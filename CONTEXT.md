...
- **26 Sep 2026, RESOLVED: USB MAIN/CUE on the MKI.** With the OCTATRIK9D diagnostic Tim saw the DC marker on 17, MAIN audio on 17/18, the track-1 copy on 19 and CUE L on 20 -- the buffer at 0x80005e60 is live on the MKI and the whole producer path works. Reflashing OCTATRICK9 with a FULL power-off then gave all 20 channels (tracks, MAIN 17/18, CUE 19/20). The earlier silence was transient state after the OS upgrade without a proper power-cycle (the same rule as the stuck sequencer of 23 Sep), not a build or MKI difference. OCTATRICK9 is therefore confirmed on the MKI in full; 9D stays on the Desktop as a tool, not for use.
- **26 Sep 2026, DIRECT JUMP stress test (emulator, port binary, OTLIVE card copy, 41 measured jumps; scripts/peeks/screens in the session scratchpad djtest/):** prompted by Discord (BUKKAKEBOREALIS: "bug when abusing it with kit/pattern changes" on an octakit build; Zac Kyoti: "test with per-track settings, varying lengths, different track and master scales -- may not be dialed in"). RESULT: no stuck sequencer, no skipped or doubled master step, the start step always = counter+1 wrapped to the new length / MASTER LENGTH (unwrapped for INF), cleared after use; after a switch every track's running position (bytes 0x800064d0+t step, 0x800064e0+t LED index, 0x800064f0+t tick) equals the destination's clean run at the same master step for 4 bars in every same-scale case -- PATTERN SCALE 16<->13<->64 from steps 3/11/15/last, PER TRACK (T1 16 1x, T2 12 3/2x, T3 64 2x, T4 7 1x; M.LEN 32 and INF), master scale 2X, 16 changes on 16 consecutive steps, same-step re-queue, cross-bank, STOP with a jump pending, restarts after a jump, CHAIN AFTER back to PAT.LEN = stock. TWO SMALL DEVIATIONS, both from the stock positioning/countdown code the arranger uses (0x400a47f6.., countdown bytes 0x800065c3+t), which stock pattern-end changes never exercise because their start is 0: (a) a track faster than the master whose step is mid-way at the switch (start ticks not a multiple of its ticks-per-step) fires its next step 2 ticks (~41 ms at 120 BPM) early, then re-locks within one step; (b) a track whose SCALE differs between old and new pattern AND whose planted position has a non-zero tick residue runs a constant 1 tick (~21 ms) early until the next MASTER LENGTH restart (with INF: until the next change) -- measured A04 -> A06 at the last step, T3 2x -> 3/4x. Fix would need the per-track tick residue planted with the position (not done; report to Tim). AUDIO HAND-OVER: selected during step n, the OLD pattern still sounds step n+1 at the switch tick and the NEW pattern's first sounding step is n+2 (continuous, nothing skipped or doubled); the pattern number and counter change one step before the new pattern sounds -- the README's "starts at the next step" is one step optimistic (corrected in the README). NOT RUN: an arranger row with the same OFFSET as a comparison; the INF counter wrap after 65535 steps (dj_queue stores counter+1 as a long; at 65535 the change-at 65536 is never reached until the next selection re-arms -- ~2 h 16 min of play at 120 BPM). Rig: 768-byte peeks back to back starve the emulator (~10x slow) -- sleep between peeks; [BANK]+[TRIG n] opens BANK n: SELECT PTN, the next trig selects.
- **26 Sep 2026, DIRECT JUMP across PARTS (emulator, 3 Parts with different machines/slots/FX/AMP, patterns A01 P1 / A02 P2 / A03 P3 / A04 P1; scripts and captures in the session scratchpad djpart/):** single jumps from steps 3/11/last, chained jumps every 2 steps across the three Parts, a jump in the first second after PLAY, a jump into STATIC machines, and an unsaved Part edit round trip all behave like stock Part changes (Part byte 0x80000003 / mirror 0x100b14cf flips in the same sample as the playing pair at the next tick 0; the destination Part's samples/PTCH/VOL sound from the step after the switch step within 0.2 dB of a clean run; counters continuous; no dropouts, freeze or DSP fault; the edit survives exactly as under PAT.LEN). ONE LIMIT: a Part change on EVERY step (alternating two patterns on different Parts step after step) never lets the second Part's voices sound -- every other step is silent (125 ms gaps) while the alternation lasts, recovery 1-3 steps after it stops; same-Part per-step alternation and every-2-step Part alternation are lossless; sequencer record, LEDs, screen, CPU and DSP counters show nothing wrong; mechanism not identified (the Part apply reaching the DSP is spread over steps). Stock has no reference (PAT.LEN changes Parts only at the bar end). Likely BUKKAKEBOREALIS's "abusing it with kit/pattern changes". Stock facts confirmed: after any Part change the live FX-id arrays 0x80000ec4/0x80000ecc and level 0x80000c50 are republished one track per step (~8 steps); the DIRECT switch step sounds the old Part, PAT.LEN's bar-end switch sounds the new Part at its tick. NOT RUN: MIDI program change (no MIDI injection in the panel server). TODO: note the every-step limit in the direct-jump README after the modules repo restructure.
- **26 Sep 2026, SUBMODULE RESTRUCTURE (prepared, PR NOT opened -- Tim's permission required before any contact):** Sam offered inclusion ("PR em ... could be PR'd in directly"); Tim chose the submodule route. (1) `timhastie/octatrick-modules` (public, MIT): direct-jump/, quantizer/, synth/ with location-independent manifests (`_HERE = os.path.relpath(dirname(realpath(__file__)))`), .s files byte-identical to cc5d782; main 0b1bdc5 = tag `v9` (the OCTATRICK9 state; direct-jump README carries the per-track and across-Parts measurements). (2) Fork `timhastie/octabam` of sambanks/octabam, branch `octatrick-modules` (f294879, base upstream main 4551989): three .gitmodules entries (same URL) at modules/<name>/upstream pinned to v9, wrapper manifest.py (runpy of upstream/<name>/manifest.py, re-exports MODULE) + README per module, remixes/octatrick.py + octatrick-usb.py, selftest _want line, README table row, docs/remixes/octatrick.md + row. Byte-neutral proof: same base built with wrappers vs plain copies -> identical (octatrick-usb 1,118,433 B sha1 7a83d4a3, octatrick 1,116,368 B sha1 08c14f66), and identical to OCTATRICK9/OCTATRIK9N despite the newer base (upstream's 0e93543..4551989 touches nothing these remixes carry); make check both remixes ALL GATES PASSED; selftest clean. Independently verified from fresh clones (PASS). PR body at ~/Desktop/Octatrick/PR_BODY.md (and scratchpad octatrick-pr/). (3) `timhastie/octatrick` main = octa-panel `upstream-port` = 35123cf: module copies replaced by the same wrappers + submodules (v9), clone instructions with --recurse-submodules, builds byte-identical to OCTATRICK9. Worktrees: scratchpad/octabam-pr (PR branch), scratchpad/upstream-port (fork), scratchpad/octatrick-modules. To publish a module change now: commit in octatrick-modules, tag, bump the three pins in octatrick (and in octabam via a PR). Remote `fork` added to this checkout.
- **26 Sep 2026, PR OPENED with Tim's explicit go: https://github.com/sambanks/octabam/pull/458** (head timhastie:octatrick-modules 26498cd, base main; body = the prepared PR_BODY). Module order everywhere is now FM synth, scale quantizer, direct jump (Tim's wish); octatrick-modules tag v9 = 2d272e2, pins bumped in the fork branch and octatrick main (946c16d); image bytes unchanged (sha1 7a83d4a3). No Discord post made; Tim has a draft line for the octabam channel if he wants it.
- **26 Sep 2026, SAM'S PR REVIEW + OCTATRIK10:** Sam (PR #458 comment): gates green on his side; finding: synth/page.s `pg_desc` was a copy of the stock FLEX PLAYBACK descriptor (0x400d31ae, 402 B, 347 identical) and sat in PINNED_PAGE too -- a "never an Elektron byte" violation; also UNFLASHED docstrings, and the shared .gitmodules URL noted. FIX (branch `page-fix` in the wt-page worktree; submodule commit d7b768e of octatrick-modules, NOT pushed): the page is cloned at first use from FLEX_P into the poly DRAM unit (po_pgdesc, reached from pg_resolve via a pointer word published just before sy_render, read through the kind-table FLEX entry 0x400d6438), page.s passes an override list (title, RATO/INDX/FDBK/DEC, formatter/widget pointers, nibbles 0x55551551); pg_desc deleted; INDX/FDBK use the stock "%d" formatter 0x4003c178; page cave 1,672 B (was 1,948; 336 B headroom before SCALE_AT now). PINNED_PAGE regenerated. Stock scan (wt-page/stock_scan.py): synth data hits 745+330 -> 0; remaining matches are hook-site replays and instruction idioms. Verified independently (PASS): 20 screens pixel-identical to OCTATRICK9, pitch exact, make check ALL GATES PASSED, DSP payloads stock. Packaged as OCTATRIK10 (`make image REMIX=octatrick-usb BUILD=10 VERSION=OCTATRIK10` in wt-page; OS body sha1 cf081545, 1,118,495 B) into ~/Desktop/Octatrick/ for Tim to test BEFORE anything is pushed or Sam is answered (Tim's wish). Draft reply to Sam agreed with Tim (not sent). Tuner and SCALE/GLIDE-persistence jobs still running in wt-tuner / wt-persist. Memory: no-elektron-bytes-in-sources.
- **26 Sep 2026, SCALE/GLIDE NOT SURVIVING A POWER CYCLE -- ROOT CAUSE + FIX (verified PASS, branch task-p-persist, octatrick-modules ba3b729, not pushed):** a power-on reads NO project file: boot (0x4001fa24..) validates battery-backed CS1 RAM (checksum 0x100fff00 over 0x100fff04..ff), sanitises the settings block 0x100b1480..0x100b14e1 (0x4000fec8 via 0x40025770) and memcpys it to the live copies (0x100b1480->0x80000020, 0x100b14cc->0x80000000). CHAIN AFTER survives because its setter/loader write the mirror 0x100b14ae. Our SCALE lived in the ROM-cave unit and GLIDE in the pinned OS byte 0x400d2cdc, both reloaded fresh from flash at power-on -> OFF. project.work is only read by CHANGE/RELOAD; SAVE = serializer 0x40088288 -> project.work then 0x4008ee74 copies every .work to .strd; SYNC rewrites .work only; RELOAD copies .strd over .work and loads; our # lines were always written fine. The emulator hid it (RAM empty at start + the rig loads the project). FIX: the settings are now battery bytes -- SCALE 0x100b14ec, GLIDE 0x100b14ed (linker padding 0x100b14e2..ef, unreferenced, outside both memcpy'd blocks; watchmem-proven); readers/writers use them in place (lea absolute), glide.s/qzg unit removed, synth GLIDE_AT = 0x100b14ed; two jsr detours: qz_boot at 0x40010212 in stock's sanitiser (out-of-range -> OFF, displaced tst.b last to keep CCR) and qz_defaults at 0x40025ac2 in the project defaults (OFF on cold boot / new project / before a load's storing pass). qz unit 3,000 B, 424 B cave left. Verified with a warm-boot rig (ot_emu env hooks OT_SRAM_IN/OUT, OT_NO_LOAD, OT_WATCHMEM, OT_WRITES_OUT -- test-rig only): OCTATRICK9 loses SCALE/GLIDE after SAVE + power cycle, keeps DIRECT; fixed build keeps all three; SAVE/SYNC/SAVE+RELOAD/CHANGE all fine; stock-save diff = the two # lines + CHAIN only; garbage clamp OK; gates green. Verifier note: the builder's binary was stale (poly still read 0x400d2cdc) but the source is right; the verifier's own build passed.
- **26 Sep 2026, TUNER MODULE (verified PASS, fork branch tuner f3f4ca0, plain modules/tuner dir; not pushed, not flashed):** UP held + TEMPO opens a 96x56 window (the stock TEMPO window's class 5, via 0x4005829c) with the current audio track's note+octave (big font), a +-50 cent needle, signed cents and Hz, ~7 readings/s; TEMPO/YES/NO/UP+TEMPO close it; HOLD keeps the last reading on silence; the stock TEMPO window is NOT opened by the chord (detour at 0x40059ef0 `tstl 0x460d16a0`: UP held = panel row byte 0x46100b18[6] bit 3), TEMPO alone / FUNC+TEMPO / UP alone stock. Frame hook 0x4000d99a (the instruction before USB AUDIO's 0x4000d9a0) copies 16 mono samples/block of the selected track (byte 0x80000000 & 7) from the read-back arena into a 4096-word ring only while open (~195 instr/block, 4 when closed); analysis in the UI task loop head (detour 0x40056c72) every >= 400 frames: McLeod NSDF at 11,025 Hz + YIN refine at 44.1 kHz, integer only (no EMAC: the scheduler saves no EMAC state), ~1.0-1.15 M instr/update. DRAM unit 2,846 B text + 19,500 B data; no ROM cave used. Measured: 16 tones 55 Hz..2 kHz right note, max 1.0 cent error; synth PTCH +12/+9/+7/+5/0/-9/-12 exact semitones; audio with the window open gap-free; make check ALL GATES PASSED. Known: inharmonic FM (RATO 1.25, high INDX) wanders ~1 st; RATO 0.25 reads the sub-octave; drums wander; UP+TEMPO while the stock TEMPO window is open only closes it (second chord opens the tuner); FUNC+TEMPO tap tempo unobservable in the port (DMA timer reads 0) but the window and tap counter behave. Remix octatrick-tuner = octatrick-usb + TUNER.
- **26 Sep 2026, OCTATRIK10 ON HARDWARE:** Tim flashed it on his MKI: "it looks fine" -- the runtime page clone (Sam's review fix) is hardware-confirmed. OCTATRIK11 (page fix + battery-RAM SCALE/GLIDE + TUNER) is being integrated/verified for his next test; PR #458 update and the reply to Sam wait for that.
- **26 Sep 2026, PR #458 UPDATED with the page fix (Tim's go):** octatrick-modules main = d7b768e, tag `v9.1` pushed; PR branch octatrick-modules e4c94a2 (pins v9.1, wrapper READMEs/manifests say v9.1 + the clone, synth README describes it) pushed to the fork -- octatrick-usb BUILD=10 on the PR base rebuilt to sha1 cf081545 = the verified page-fix build; PR body updated (pin, 1,672 B page, "Review fix" section); Tim's reply posted as comment 5846496926. octatrick main (upstream-port) NOT bumped yet -- it follows with v10 (OCTATRIK11 integration running). The persistence fix and the tuner are NOT in the PR yet ("do the rest later").
- **26 Sep 2026, octatrick-modules HISTORY REWRITTEN (Tim ran the force-push himself; the classifier refused it for me):** the repo now has ONE commit, main = v9.1 = 42877ad (tree identical to the page-fix commit d7b768e); tag v9 deleted; old history kept only in ~/Desktop/Octatrick/octatrick-modules-old-history.bundle (never pushed). No commit on GitHub carries stock OS bytes any more (GitHub may still serve the old objects by hash for a while). PR branch re-pinned to 42877ad (6d1479c, pushed; PR body hash updated); octatrick main / octa-panel upstream-port re-pinned to 42877ad with the wrapper docs (see the commit). The v10 integration (running in wt-v10) was cloned from the OLD history: its v10 result must be re-committed on top of 42877ad (tree copy), not merged.
- **26 Sep 2026, OCTATRIK11 packaged for Tim (v10 integration; independent verifier STOPPED early at Tim's request -- builder smoke tests are the evidence):** octatrick-modules v10 = 98b43c5 in scratchpad/mods-v10 (merge of page-fix + task-p-persist onto the OLD history + tuner/ + tools/stock_scan.py; NOT pushed; must be re-created as a commit on top of the clean root 42877ad before pushing -- tree copy, not merge). Fork branch v10 (wt-v10, e83ebf5): four submodules (tuner added), remix octatrick-tuner; the persist test-rig ot_emu hooks are uncommitted there (patch scratchpad/wt-v10-otemu-rig.patch) -- never push them. Images: octatrick-tuner BUILD=11 OS body sha1 277f8be8 (1,120,411 B; 6 DRAM units; 424 B cave left), octatrick-usb BUILD=11 sha1 bdf1ddd8; OCTATRACK_OCTATRIK11.bin/.syx in ~/Desktop/Octatrick/ (DSP payloads stock, syx section 3 == body, checksums ok). Builder smoke: 18 screens pixel-identical to OCTATRIK10, pitch exact, GLIDE 64 legato from the battery byte, SCALE snapping, SAVE + warm boot keeps SCALE/GLIDE/CHAIN, tuner UP+TEMPO reads C4 +0c / C5 / E3 and closes, TEMPO alone stock, make check both remixes ALL GATES PASSED, stock scan 0 data hits. Leftover: quantizer README intro still says 'one pinned 4-byte unit (the GLIDE byte)' (stale). Next after Tim's hardware test: re-create v10 on the clean history, push + tag, bump pins in octatrick main; PR pin bump only with Tim's go.
- **26 Sep 2026, VOIC = VOICE CAP (verified PASS; octatrick-modules branch voic-cap c228c6e in mods-v10, fork v10 2dfe5b6; not pushed):** Tim's hardware report (VOIC 2 still gave 4 keyboard voices; a 2-note shape identical at VOIC 2/4) matched the old definition (VOIC = chord notes per start, pool always 4). New: VOIC n = the most voices the track sounds at once, keys and chords alike: po_start counts active voices and cuts the oldest (releasing before sounding, V_AGE) so active + k <= n, k = min(shape notes, n); VOIC 1 = mono path untouched; release tails count toward the cap (chord memory). +71 B runtime. Measured (T1): VOIC 2 four keys -> the two newest; VOIC 3 -> 3; VOIC 4 -> 4; MAJ at VOIC 2/3/4 = 2/3/3 notes; 4TH = 2 at every VOIC; safety nets clean. FOUND: quantizer.s qz_polytrack used track*40 instead of track*24 for the Part LFO-page offset -> on T2..T8 CHROMATIC keys reached the paraphonic engine only if an unrelated byte read 2..4 (on the OTLIVE card T2 never posted keys) -- fixed in the next job. Also noted: 2-4 poly voices in phase clip the mix (intermod -27 dB at 2 voices, -14 dB at 4) -- the phase-5 level design (each voice 1/2, sum doubled + saturated); unchanged, to raise with Tim. NEXT JOB (running, branch tuning): chord priority order per shape; PTCH = +-64 semitones on synth tracks (1 st/unit), CHROMATIC octaves -4..+4 with exact recorded locks, RATE -> FINE (+-64 cents), qz offset fix; then OCTATRIK12.
- **26 Sep 2026, CHORD PRIORITY + TUNING SYSTEM + qz OFFSET FIX (verified PASS; octatrick-modules branch tuning 9e27c65 in mods-v10, fork v10 9c58c2f; not pushed):** (A) qz_polytrack track*40 -> track*24: CHROMATIC keys are paraphonic on every track. (B) po_shapes rows in priority order (root, naming note, 3rd, 5th; stacks/inversions/spreads lowest first): MAJ6 VOIC 2 = C4+A4, VOIC 3 = C4 E4 A4; MAJ7 VOIC 3 = C4 E4 B4; DIM7 VOIC 2 = C4+F#4. (C) SYNTH TRACKS ONLY: PTCH = semitones, 1/unit, raw 64 = 0, -64..+63, page prints "+7"; engine sy_word: w = 0x4000 + (ptch-0x4000)*5 + (fine-0x4000)/20, po_rate_fold folds by octaves on both paths (RATE never scales a synth voice), INC_MAX 19.8 kHz; SCALE snapping in semitone units (qz_pcof) on the knob/p-lock hooks and pitch-class snap in qz_chrom; CHROMATIC octave = qz_oct -4..+4 (FUNC+LEFT/RIGHT; five new jmp detours 0x40050254 / 0x40045918 / 0x40044968 / 0x400449b8 / 0x4000e74c; stock's 0/1 word held at 0 on synth tracks; the number beside the keyboard prints qz_oct); live recording writes the exact raw (octave +2 key 13 -> lock 88 = +24 -> 1046.5 Hz; -3 -> 28 = -36 -> 32.7 Hz); RATE -> FINE (-64..+63 cents, "+20c", default 64 in the clone; +20c = 264.6 Hz at C4); MIDI IN chromatic on synth tracks maps raw = note-20 but the stock 72..96 note gate stays (extended range not reachable over MIDI); chromatic note OUT stays 72+key whatever qz_oct (two more detours would be needed; 40 B cave left). Sample tracks stock (their chromatic octave is a 0/1 toggled word, not -1..+1). Measured: octaves -2/-1/0/+1/+3/+4 = 65.4/130.8/261.6/523.2/2093/4186 Hz; PTCH +7 = 391.9 Hz; SCALE MAJOR snaps on the extended keyboard; GLIDE C4 -> C6 legato; sample track T5/T7 stock. Bus 4,781 B changed; quantizer unit 3,404 B, page cave 1,800 B (PINNED_PAGE re-pinned); ROM CAVE 40 B LEFT (next quantizer growth needs DRAM or trimming: qz_hold128 duplicates poly's table). Stock scan zero data hits. CAVEAT for Tim: a synth track converted from a sample track carries RATE 127 = FINE +63c -> set FINE to 0c once per such track. NEXT (running): per-voice gain = mono/VOIC so chords never clip; then OCTATRIK12 final.
- **26 Sep 2026, POLY LEVEL (verified PASS) + OCTATRIK12 PACKAGED:** poly.s: V_GAIN Q15, T_GMAX = 32768 / VOIC latched per start (16384/10922/8192), ramp T_GMAX>>3 a frame, release clamped to T_GMAX, sum doubled as the mono path -> VOIC in-phase voices peak exactly at the mono full scale, saturation never fires; single note -6.02/-9.54/-12.04 dB re mono at VOIC 2/3/4; OCT3 at VOIC 4 peaks -18.2 dBFS with no product above -75 dB (was intermod -14 dB). Note: the OTLIVE T2 has a resonant FILTER (Q 94) that lifts C5 +15.6 dB over C4 -- project, not engine. octatrick-modules branch tuning = 19e20f3 (mods-v10), fork v10 = ee72da0 (wt-v10), NOT pushed. OCTATRIK12 = octatrick-tuner BUILD 12 (OS body in ~/Downloads/octabam/out/mainos_octatrik12.bin; card .bin sha1 a35d710c) in ~/Desktop/Octatrick/ with the WHICH_FILE entry (FINE 0c reset on existing synth tracks!). PUBLISH PLAN after Tim's hardware test: recreate the v10 state as ONE commit on the clean root 42877ad (tree copy of 19e20f3), tag v10, push; bump pins in octatrick main / octa-panel upstream-port (+ tuner submodule, remix octatrick-tuner); PR pin bump only with Tim's go (the PR would then carry persistence + tuning + VOIC changes; the tuner stays out unless Tim wants it in).
- **27 Sep 2026, CHORD VOICINGS + EQUAL-POWER LEVEL + PEAK LIMITER (verified; octatrick-modules branch tuning c8ac50e, fork v10 1233fc8; not pushed):** every shape but "----" is an explicit 4-note voicing in priority order (own notes, essentials before the fifth, then octave doublings: 4TH 0,5,12,17; MAJ 0,4,7,12; MAJ7 0,11,4,7 ...); VOIC n plays the first n notes of every shape; a start with a shape is chord memory (po_free cuts the previous chord); "----" = keyboard polyphony with the oldest-cut cap. Level: T_GMAX = 32768/sqrt(VOIC) (23170/18919/16384) -> single notes -3.0/-4.8/-6.0 dB re mono; the soft-clip curve was simulated and REJECTED (intermod -17..-19 dB on ordinary chords); instead a per-frame PEAK LIMITER in po_fill (track gain T_LIM Q16, pulled to FS/peak within the frame when the sum would pass the mono full scale, releases with tau 0.74 s, unity = untouched samples): every chord peaks at the mono full scale (+0.03..0.15 dB DSP ripple); OCT3 VOIC 4 products -61 dB, 4TH VOIC 2 -57 dB (a beat sideband of the gain ride; the verifier's -60 dB bound was arbitrary), MAJ VOIC 4 -46 dB, four "----" keys -37 dB. Bus 1,120,702 B, ledger unchanged, cave 40 B. Tim's report "no difference between VOIC 2 and 4 with 4TH" and "level drop too drastic" both addressed. NEXT (running): tuner "--" placeholder centred, FMSYNTH.wav accepted as the marker name (SYNTH.wav kept), marker sample position pinned (length/loop irrelevant) -> OCTATRIK13.
- **27 Sep 2026, OCTATRICK 2.3 PACKAGED (new naming scheme: files OCTATRACK_OCTATRICK2.3.bin/.syx, on-screen OCTATRK2.3; BUILD 13 internal):** last batch verified: tuner "--" placeholder centred through ctext (dashes x 61..69, note x 57..73, same centre 65); FMSYNTH.wav accepted as the marker (a leading "FM" skipped before the "SYNTH" compare in poly.s x2, quantizer.s qz_is_synth (+4 B net ROM after trimming two compares), page.s pg_scanned -- PINNED_PAGE regenerated, PAGE_LEN 1812); marker position pinned (sy_render saves/restores the voice struct's position fields +64..+83 around the stock call on the synth path) -> a 4 s marker with LOOP OFF holds a note 6.5 s+, plain FLEX tracks still end, HOLD 40 trig unchanged. octatrick-modules branch tuning = f46e321 (mods-v10), fork v10 = e86f940 (wt-v10); NOT pushed. Bus 1,120,748 B (4,797 changed), cave 40 B, DSP stock, scan zero data hits. Desktop: OCTATRACK_OCTATRICK2.3.bin (sha256 7392d943...), _OS1.40C_OCTATRICK2.3.syx (fd125ef9...), FMSYNTH.wav, WHICH_FILE entry; OS body copy out/mainos_octatrick2.3.bin. IDEAS discussed, not built: chord recognition when recording fingered chords (exact/inversion match -> CHRD+VOIC+PTCH locks; nearest only if it contains every played note); per-step independent notes (needs new step storage); MIDI-track-driven polyphony (needs poly MIDI-in + internal routing) -- judged inconvenient.
- **26 Sep 2026 13:57 UTC, PR #458 MERGED by Sam** (merge commit 0b6204c on sambanks/octabam main): modules/direct-jump, quantizer, synth as wrappers + submodules of timhastie/octatrick-modules pinned at v9.1 (42877ad), remixes octatrick / octatrick-usb, docs. CI on the PR head 6d1479c: all six jobs green (the earlier e4c94a2 run "no jobs were run" was a GitHub setup failure, superseded). From now on a release = tag in octatrick-modules + a pin-bump PR to octabam (three gitlinks in one commit) -- only with Tim's go. The octatrick fork (base 0e93543) can later rebase onto upstream main, which now carries the modules itself.
